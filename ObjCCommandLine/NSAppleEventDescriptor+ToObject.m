...
- (id)toObject {
    ...
    switch (desc) {
        case typeAEList: {
            ...
            unsigned int i = 1;
            // AS lists are 1-indexed.
            for (i = 1; i <= [self numberOfItems]; i++) {
                id obj = [[self descriptorAtIndex:i] toObject];
                if (obj) {
                    [ret addObject:obj];
                }
            }
        ...
        default:
            ...
            NSLog(@"Unknown AEType: '%s%C%C%C%C'", __PRETTY_FUNCTION__, (unichar)(desc >> 24 & 0xFF), (unichar)(desc >> 16 & 0xFF), (unichar)(desc >> 8 & 0xFF), (unichar)(desc & 0xFF));
            ret = [self stringValue];
    }
    return ret;
}
